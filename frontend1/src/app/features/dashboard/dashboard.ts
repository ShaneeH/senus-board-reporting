...
export class Dashboard implements OnInit {
  ...
  reportDocuments: FinancialDocument[] = [];
  ...
  onCompanySelected(company: DropdownOption<number>): void {
    ...
    this.loadDocuments(company.value);
  }

  get selectedReportDocuments(): FinancialDocument[] {
    if (!this.selectedPeriod) return [];

    return this.reportDocuments.filter(document =>
      document.periods.some(period => period.period === this.selectedPeriod?.value)
    );
  }
...
private loadCompanies(): void {
  ...
  private loadDocuments(companyId: number): void {
    this.companyService.getDocuments(companyId).subscribe({
      next: documents => {
        this.reportDocuments = documents;
      },
      error: error => console.error('Failed to load report exports.', error)
    });
  }
}
